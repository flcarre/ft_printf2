...
/*   Updated: 2019/02/27 12:02:14 by flcarre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_2(t_id *e, unsigned long long x)
{
	...
	(r[1] = ft_iputunbr_base(x, "0123456789abcdef")) >= 0)
		r[0] += r[1];
	else if (r[1] >= 0 && e->id[0] == 'o' && x != 0 && \
	(r[1] = ft_iputunbr_base(x, "01234567")) >= 0)
		r[0] += r[1];
	else if (r[1] >= 0 && e->id[0] == 'X' && \
	(r[1] = ft_iputunbr_base(x, "0123456789ABCDEF")) >= 0)
		r[0] += r[1];
	else if (r[1] >= 0 && (r[1] = ft_iputunbr_base(x, "0123456789")) >= 0)
		r[0] += r[1];
	r[0] = (r[1] < 0) ? r[1] : r[0];
	return ((int)r[0]);
}
...
int			ft_pprocess3(t_id *e, unsigned long long x)
{
	...
	if ((r[1] = ft_iputfm(e, x)) >= 0)
		r[0] += r[1];
	e->p[0] -= (e->id[0] == 'o' && x == 0 && (e->fm & 16) == 16 && \
	e->p[0] > 0) ? 1 : 0;
	if (e->id[0] == 'o' && r[1] >= 0 && e->p[1] && x != 0 &&\
	...
	(r[1] = ft_iputxchar('0', e->p[0] - ft_udigitlen(x, 16))) >= 0)
		r[0] += r[1];
	else if (r[1] >= 0 && e->p[1] && e->p[0] > ft_udigitlen(x, 10) && \
